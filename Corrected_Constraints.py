...
class VRPTWOptimizer:
    ...
    def _add_la_arc_constraints_with_parsimony(self):
        ...
        for u in self.customers:
            ...
            # Create k-indexed neighborhoods
                        ...
                        if (w,v) in self.E_star:
                            self.model.addConstr(
                                rho * k + self.x[w,v] >= gp.quicksum(
                                    ...
                                    if self._is_in_k_neighborhood_ordering(u, r, w, v, k, N_k_u[k])
                                ),
            ...
            for k in range(1, len(distances) + 1):
                for w in N_k_plus_u[k]:
                    self.model.addConstr(
                        rho * k + gp.quicksum(
                            self.x[w,j] for j in (self.customers + [self.depot_end])  # N^+
                            if j not in N_k_plus_u[k] and (w,j) in self.E_star
                        ) >= gp.quicksum(
                            self.y[u,r] for r in range(len(self.R_u[u]))
    ...
    def _add_flow_thresholds(self):
        ...
        print("\nz_D variables and values:")
        for ((u1, k1), (u2, k2)), var in self.z_D.items():
            if var.X > 1e-6:
                print(f"Flow from ({u1},{k1}) to ({u2},{k2}): {var.X}")
                print(f"Buckets for {u1}: {self.D_u[u1] if u1 in self.D_u else 'not found'}")
                    
        print("\nAttempting to process flows...")
        # Get flow values with significance threshold
        z_T_vals = {}
        z_D_vals = {}
        for ((u1, k1), (u2, k2)), var in self.z_D.items():
            if var.X > 1e-6:
                z_D_vals[((u1,k1), (u2,k2))] = var.X
                    
        for ((u1, k1), (u2, k2)), var in self.z_T.items():
            if var.X > 1e-6:
                z_T_vals[((u1,k1), (u2,k2))] = var.X

        # Process flows with safe bucket access
        for ((u_i, k_i), (u_j, k_j)), flow in z_D_vals.items():
            if u_j not in [self.depot_start, self.depot_end]:
                if k_i >= len(self.D_u[u_i]):
                    k_i = len(self.D_u[u_i]) - 1  # Use last available bucket
                    
                # Get bucket bounds
                d_plus_i = self.D_u[u_i][k_i][1]
                new_threshold = d_plus_i - self.demands[u_i]
                
                # Add threshold if meaningful and not present
                if (self.demands[u_j] < new_threshold < self.vehicle_capacity and
                    not any(abs(bucket[1] - new_threshold) < 1e-6 for bucket in self.D_u[u_j])):
                    self.D_u[u_j].append((new_threshold, self.vehicle_capacity))
                    self.D_u[u_j].sort(key=lambda x: x[0])  # Keep buckets ordered
                    changes_made = True
                    
                print(f"Processing flow {u_i},{k_i} -> {u_j},{k_j}")
                print(f"d_plus_i: {d_plus_i}, new_threshold: {new_threshold}")
                print(f"Updated buckets for {u_j}: {self.D_u[u_j]}")

        # Similarly handle time buckets
        for ((u_i, k_i), (u_j, k_j)), flow in z_T_vals.items():
            if u_j not in [self.depot_start, self.depot_end]:
                if k_i >= len(self.T_u[u_i]):
                    k_i = len(self.T_u[u_i]) - 1
                    
                t_plus_i = self.T_u[u_i][k_i][1]
                travel_time = self.costs[u_i,u_j]/5
                new_threshold = t_plus_i - travel_time
                
                # Add new threshold if meaningful and not present 
                if (self.time_windows[u_j][0] < new_threshold < self.time_windows[u_j][1] and
                    not any(abs(bucket[1] - new_threshold) < 1e-6 for bucket in self.T_u[u_j])):
                    ...
                    for k, bucket in enumerate(self.T_u[u_j]):
                        if new_threshold < bucket[1]:
                            ...
                            if new_threshold > bucket[0]:
                                # Replace, don't append duplicate entries
                                new_buckets = self.T_u[u_j][:k]
                                new_buckets.append((bucket[0], new_threshold))
                                new_buckets.append((new_threshold, bucket[1]))
                                new_buckets.extend(self.T_u[u_j][k+1:])
                                self.T_u[u_j] = new_buckets
                                changes_made = True
                            break
                    
                print(f"Processing time flow {u_i},{k_i} -> {u_j},{k_j}")
                print(f"t_plus_i: {t_plus_i}, new_threshold: {new_threshold}")
                print(f"Updated time buckets for {u_j}: {self.T_u[u_j]}")
    ...
    def solve_with_parsimony(self, time_limit=None):
        ...
        while True:
            ...
            if self.model.Status == GRB.OPTIMAL:
                ...
                print(f"LP optimization failed with status {self.model.Status}")
                return None
        ...
        if self.model.Status == GRB.OPTIMAL:
            ...
            print(f"MILP optimization failed with status {self.model.Status}")
            return None
    ...
    def _is_in_k_neighborhood_ordering(self, u, r, w, v, k, N_k_u):
        """Check if a^k_{w,v,r} = 1"""
        ordering = self.R_u[u][r]
        sequence = ordering['sequence']
        
        # First check if w immediately precedes v
        for i in range(len(sequence)-1):
            if sequence[i] == w and sequence[i+1] == v:
                # Now check if all customers up to and including v are in k-neighborhood
                for j in range(i+2):  # Check up through position of v
                    if sequence[j] not in N_k_u:
                        return False
                return True
        return False
