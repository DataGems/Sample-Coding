...
def generate_la_arcs(customers, neighbor_sets, costs, time_windows, service_times, demands, vehicle_capacity):
    ...
    def get_earliest_completion(sequence):
        """Calculate earliest possible completion time for sequence"""
        current_time = time_windows[sequence.start][0]  # Earliest possible start
        
        for i in range(len(sequence.sequence)-1):
            current = sequence.sequence[i]
            next_customer = sequence.sequence[i+1]
            
            # Add service time
            current_time += service_times[current]
            
            # Add travel time
            current_time += costs[current, next_customer]
            
            # Update to earliest possible service time at next customer
            current_time = max(current_time, time_windows[next_customer][0])
            
        # Add final customer's service time
        current_time += service_times[sequence.sequence[-1]]
        return current_time

    def get_latest_start(sequence):
        """Calculate latest possible start time that maintains feasibility"""
        # Start from latest possible time at last customer
        current_time = time_windows[sequence.sequence[-1]][1]
        
        # Work backwards through sequence
        for i in range(len(sequence.sequence)-1, 0, -1):
            current = sequence.sequence[i]
            prev_customer = sequence.sequence[i-1]
            
            # Subtract service time of previous customer
            current_time -= service_times[prev_customer]
            
            # Subtract travel time
            current_time -= costs[prev_customer, current]
            
            # Update to latest possible time at previous customer
            current_time = min(current_time, time_windows[prev_customer][1])
    ...
    def is_time_feasible(sequence):
        """Check if sequence satisfies time window constraints"""
        # Start with earliest possible time at first customer
        current_time = time_windows[sequence.sequence[0]][0]
        first = sequence.sequence[0]
        last = sequence.sequence[-1]
        
        # Forward pass - check if we can reach all customers within their time windows
        for i in range(len(sequence.sequence)-1):
            current = sequence.sequence[i]
            next_customer = sequence.sequence[i+1]
            ...
            # Check if we can service next customer
            earliest_service = max(current_time, time_windows[next_customer][0])
            if earliest_service > time_windows[next_customer][1]:
                return False
                
            current_time = earliest_service
        
        # Check if overall sequence timing works
        earliest_completion = current_time + service_times[last]
        latest_start = time_windows[first][1]
        
        return earliest_completion <= time_windows[last][1] and \
            current_time >= time_windows[first][0]
    ...
    def generate_sequences(start_customer, available_neighbors, end_customer):
        ...
        if not available_neighbors:
            ...
            if (is_capacity_feasible(sequence) and is_time_feasible(sequence)):
                return [sequence]
            return []
        ...
        direct = LASequence(start_customer, [], end_customer,
                          costs, time_windows, service_times)
        if (is_capacity_feasible(direct) and is_time_feasible(direct)):
    ...
    for u in customers:
        ...
        for v in non_neighbors:
            ...
            for seq in sequences:
                ...
                logger.debug(f"Earliest completion: {get_earliest_completion(seq):.2f}")
                logger.debug(f"Latest start: {get_latest_start(seq):.2f}")
                ...
                for existing in efficient_sequences:
                    if is_dominated(seq, existing):
                        ...
                        logger.debug(f"  Existing - dist:{existing.total_distance:.2f}, " + \
                                   f"compl:{get_earliest_completion(existing):.2f}, " + \
                                   f"start:{get_latest_start(existing):.2f}")
