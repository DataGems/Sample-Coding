...
class VRPTWOptimizer:
    ...
    def _add_flow_thresholds(self):
        """Add new thresholds based on flow solution (lines 16-17 of Algorithm 1)"""
        changes_made = False
        
        # Get flow values from model
        z_T_vals = {e: var.X for e, var in self.z_T.items()}
        z_D_vals = {e: var.X for e, var in self.z_D.items()}
        
        # Add capacity thresholds based on flow (line 17)
        for (i, j), flow in z_D_vals.items():
            if flow > 1e-6:  # Only consider significant flows
                u_i, k_i = i  # Source node customer and bucket
                u_j, k_j = j  # Target node customer and bucket
                
                if u_j not in [self.depot_start, self.depot_end]:
                    # Calculate new threshold d^+_i - d_{u_i} 
                    ...
                    # Add new threshold if meaningful and not already present
        ...
        # Add time thresholds based on flow (line 16)
        for (i, j), flow in z_T_vals.items():
            if flow > 1e-6:  # Only consider significant flows
                u_i, k_i = i  # Source node customer and bucket
                u_j, k_j = j  # Target node customer and bucket
                
                if u_j not in [self.depot_start, self.depot_end]:
                    # Calculate new threshold min(t^+_i - t_{u_i,u_j}, t^+_j)
                    t_plus_i = self.T_u[u_i][k_i][1]  # Upper bound of source bucket
                    t_u_i_u_j = self.costs[u_i,u_j]/5  # Travel time between customers
                    t_plus_j = self.T_u[u_j][k_j][1]  # Upper bound of target bucket
                    new_threshold = min(t_plus_i - t_u_i_u_j, t_plus_j)
