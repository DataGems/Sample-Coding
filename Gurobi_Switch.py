...
class LASequence:
    ...
    def is_time_feasible(self):
        current_time = self.time_windows[self.start][0]
        
        logger.debug(f"\nChecking time feasibility for sequence {self.sequence}")
        logger.debug(f"Starting at time {current_time}")
        
        for i in range(len(self.sequence)-1):
            current = self.sequence[i]
            next_customer = self.sequence[i+1]
            
            # Add service time at current customer
            current_time += self.service_times[current]
            logger.debug(f"After service at {current}: time = {current_time}")
            
            # Add travel time to next customer
            travel_time = self.costs[current, next_customer]
            current_time += travel_time
            logger.debug(f"After travel to {next_customer} (+{travel_time}): time = {current_time}")
            
            # Check if we can feasibly arrive at next customer
            if current_time > self.time_windows[next_customer][1]:
                logger.debug(f"Failed: Arrival at {next_customer} too late "
                           f"(window ends at {self.time_windows[next_customer][1]})")
                return False
                
            # Update to earliest possible service time at next customer
            current_time = max(current_time, self.time_windows[next_customer][0])
            logger.debug(f"Updated start time at {next_customer}: {current_time}")
            
        logger.debug("Time feasibility check passed")
        return True

def generate_la_arcs(customers, neighbor_sets, costs, time_windows, service_times, demands, vehicle_capacity):
    """Generate LA-arcs for each customer"""
    la_arcs = {}
    ...
    for u in customers:
        ...
        logger.debug(f"Non-neighbors: {non_neighbors}")
        
        feasible_sequences = []
        
        # Generate all possible intermediate sequences
        for length in range(len(neighbors) + 1):
            logger.debug(f"\nTrying sequences of length {length+2} (including start and end)")
            
            for intermediate in itertools.combinations(neighbors, length):
                for v in non_neighbors:
                    for intermediate_perm in itertools.permutations(intermediate):
                        sequence = LASequence(u, list(intermediate_perm), v, 
                                           costs, time_windows, service_times)
                        
                        # Log detailed feasibility checks
                        logger.debug(f"\nChecking sequence: {sequence}")
                        
                        if not sequence.is_elementary():
                            logger.debug("Failed elementary check")
                            continue
                            
                        if not sequence.is_capacity_feasible(demands, vehicle_capacity):
                            logger.debug("Failed capacity check")
                            continue
                            
                        if not sequence.is_time_feasible():
                            logger.debug("Failed time feasibility check")
                            continue
                            
                        logger.debug("Sequence is feasible")
                        feasible_sequences.append(sequence)
        
        logger.info(f"Found {len(feasible_sequences)} feasible sequences")
        la_arcs[u] = feasible_sequences
