...
class VRPTWOptimizer:
    ...
    def _is_capacity_edge_valid(self, d_min_i, d_max_i, d_min_j, d_max_j, demand_i):
        """Check if edge is valid in capacity graph"""
        # Check if capacity after serving customer i can reach required capacity at j
        if d_max_i - demand_i >= d_min_j and d_min_i - demand_i <= d_max_j:
            return True
        return False
    
    def _is_time_edge_valid(self, t_min_i, t_max_i, t_min_j, t_max_j, travel_time):
        """Check if edge is valid in time graph"""
        # Check if time after serving customer i can reach required time at j
        if t_min_i + travel_time <= t_max_j and t_max_i + travel_time >= t_min_j:
            return True
        return False
